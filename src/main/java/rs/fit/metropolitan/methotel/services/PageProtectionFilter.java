...
package rs.fit.metropolitan.methotel.services;

import rs.fit.metropolitan.methotel.data.Role;
import rs.fit.metropolitan.methotel.entities.Korisnik;
import rs.fit.metropolitan.methotel.pages.Index;
import rs.fit.metropolitan.methotel.pages.Login;
...
public class PageProtectionFilter implements ComponentRequestFilter {
    ...
    private boolean isAuthenticated() throws IOException {
        ...
        if (user != null) {
            if (user.getKorisnikRola()== Role.Admin || user.getKorisnikRola()
                    == Role.Korisnik) {
                return true;
            }
        }
